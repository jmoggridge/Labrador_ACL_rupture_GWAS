...
versatile [`plink` toolset](https://zzz.bwh.harvard.edu/plink/index.shtml).
...
This tutorial can only provide a short explanation of each function and the parameters used, but you will certainly want more information: check out the (plink manual)[https://zzz.bwh.harvard.edu/plink/reference.shtml] to get more insight into each function as you work your way through the example here.
...
All commands should be run in the unix shell. Later on we will do some plotting in R.
...
**Start an srun session now**, then load these modules to get plink and R ready for the tutorial.
...
The files .imiss and .lmiss show the proportion of missing data per loci and indiv. Download these to your local machine and check the distribution if you want to adjust the thresholds. There is code for plotting these in the R section at the end of the tutorial.
...
Since we are dealing with Labrador retrievers, we'll just perform MDS to check for any obvious outliers. We also use the MDS components as covariates to control for population stratification in some association testing. We'll use the set of files named 'cr237_filter6.*' for MDS in this step. We'll plot the MDS in R too (see R section).
...
The MDS results are in `cr237_filter6.mds`. You'll want to visualize these before moving on, so download the cr237_filter6.mds and .fam files and skip to the R section for MDS plotting. Check the results and return back to here afterwards. In the first two MDS components we can vaguely see two groups, but no individuals are extremely far from the cloud. 

We don't *need* to, but we could exclude the couple controls that
are around -0.1 in coefficient 2 as an example:

```{r MDS_filter, engine = 'bash', eval = F, include = T}
# Using awk to exclude them from the list
awk '{ if ($5 >-0.07) print $1,$2 }' cr237_filter6.mds \
  > cr237_filter6_strat

# To only keep the individuals within MDS thresholds
plink --dog --bfile cr237_filter6 \
  --keep cr237_filter6_strat \
  --make-bed --out cr237_filter6_strat
```

You can see that this removed those few distant dogs. However we don't really need to do this (they aren't that distant) and we don't have any other data to reference against here (eg. other dog breeds to anchor our Labs). Removing individuals also decreases our statistical power (and we'd have to redo the MDS!).
...
We can also get various corrections for multiple testing: adjusted, Bonferroni, Holm, Sidak, and FDR stats can be obtained by calling `--assoc` with the `--adjust` flag. We get an extra file '*.assoc.adjusted' with the output.
...
Note: there aren't any (!) significant results after some corrections. Baker *et al*. note that Bonferroni correction might be too stringent given that dogs have many SNPs that are inherited in haplotype blocks due to extensive linkage disequilibrium.


### Testing with `--logisitic`
...
At this stage, you'll want to check out a Manhattan plot of the association p-values - but get the permutation running first! Then you'll have some time (~30mins) on your hands to go to the R section and do the Manhattan plot.

### Permutation testing

This involves randomly permuting the phenotypes to get estimates of  significance empirically and as such it is computationally intensive but has some desirable properties for test. This is a general procedure and various tests can be applied in this framework; [go here learn more](https://zzz.bwh.harvard.edu/plink/perm.shtml) about the different uses of plink --perm. For more background, see this [article by Che *et al.*](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4070098/).

In the context of our dataset (small size, possibly contains relatives since we did not filter based on pi_hat), permutation is an attractive option to avoid some of the assumptions of ANOVA testing. Here we do 1 million permutations of the genotypes with the `--assoc` flag and `--model` specified as `mperm` with our number of repeats. 
This will invoke the max(T) algorithm rather than the adaptive one.
You may set the `--seed` to any large integer so that you get the same results (there is randomness otherwise).
...
FYI - this will be the most computationally intensive step and will take at least tens of minutes (much longer if your dataset is large). Once it has started, it's a good time to run some of the R code included with this tutorial to explore the data we have generated already. 


---

## Challenge: Polygenic risk scores

Hurrah! You finished the entire tutorial! The next section covers plotting all the most important results in R.

You're now a pro at using plink and you're ready to start exploring more GWAS methods on your own. Calculating the polygenic risk scores for the Labradors in our data is left as an exercise for the reader (*ie.* I ran out of time to work on this). 
...
This section covers how to plot plink results with R. I prefer to use the `ggplot2` library over base R, so I will show you how to plot plink output this way. If you prefer base R plots, then see the [Marees GWAS tutorial](https://github.com/MareesAT/GWA_tutorial/). 

The data from plink are fairly easy to work with but have an usual space-delimited format and the column names have white-space when they are read in. I like the `janitor::clean_names()` function to make them all lowercase names. The functions here can be fed any data of the right type, though you may want to adjust the plot layers if your data are very different from the Labradors example. Since the figures are ggplot2 objects, you can easily customize plot appearance to your liking with the usual ggplot2 stuff: theme_\*(), theme(), labs(), *etc*. 
...
library(GGally)       # for the MDS pairs plot
...
### QC: Minor allele frequency from plink --freq
...
```{r make_mds1}
# a 2d MDS plot
fig1_gwas_mds1 <- gwas_mds1(dogs) + coord_equal()
fig1_gwas_mds1
```

You may want to check for outliers in multiple MDS components.

```{r make_mds2, fig.height=5, fig.width=6}
# multiple pairwise
...
/home/jmoggrid/scratch/lab_test_run/result2.assoc.logistic
...
snpsOfInterest <- logistic %>% 
  filter(p < as.numeric('1e-6')) %>%
  pull(snp)
         
# Make the Manhattan plot using the function we created above
# the 'label_if' parameter sets the threshold at which snps will have text labels
...
We can also get a table of the most significant associations:
...
This section creates the QQ plot for your association p-values.
...
Plot the q-q plot for the logistic regression p-values in the Manhattan above. This is showing us the distribution of our p-values against the distribution expected by chance alone. Since the fit is very close to the expected, this indicates that we don't have much in the way of strong associations.
