...
#### For more info about association testing with plink see:
https://www.cog-genomics.org/plink/1.9/assoc


### Testing with `--assoc`

We first do simple Chi-squared test with one degree of freedom using the `--assoc` function. It's important to note that this testing does not include correction for the MDS covariates (which is undesirable).
...
We do want to take the MDS covariates into account in our model though and the --assoc function doesn't do this.  We can do this with plink's `--logistic` function that fits a logistic regression model to the data with the MDS components as covariates (passed with `--covar`). We also provide the `--hide-covar` flag to only get the results for the SNPs (not the MDS covariates).
...
<!-- This creates the classic GWAS figure: the Manhattan plot. The p-values for each SNP are plotted on the y-axis, with SNP position along the x-axis. This code was adapted from [a blog post by Daniel Roelfs](https://danielroelfs.com/blog/how-i-create-manhattan-plots-using-ggplot/). -->
